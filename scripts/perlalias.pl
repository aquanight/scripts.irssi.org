...
You can persist data using an 'our' variable. These variables are not shared with other aliases, and neither are named subs that you might declare.
...
# like ^W or whatever. Even whitespace is not safe because command_runsub exists.
# They can do whatever they like with their footguns, but we're not obligated to provide them loaded.
our %alias_packages = ();
...
sub sig_setup_saved {
	...
	for my $name (keys %alias_packages) {
		my $package = $alias_packages{$name};
		no strict 'refs';
		my $text = ${"${package}::_text"};
		printf $fd "%s\t%s\n", $name, $text;
...
sub op_is_sub {
	...
	CHECK: {
		if ($op->name eq 'null') {
			my @nullkids = op_kids $op;
			return "" if @nullkids != 1;
			$op = shift @nullkids;
			redo CHECK;
		}
		elsif ($op->name eq 'gv') {
			...
			if (B::class($op) eq "PADOP") {
				$sv = $pad->ARRAYelt($op->padix)->object_2svref;
			}
			elsif (B::class($op) eq "SVOP") {
				$sv = $op->sv->object_2svref;
			}
			if (ref($sv) eq "CODE") {
				return $sv == $sub;
			}
			elsif (ref($sv) eq "GLOB") {
				return *$sv{CODE} == $sub;
			}
			else {
				return "";
...
sub collect_shared_variables {
	...
	my sub try_multi_state {
		...
		# 
		if ($op->name eq 'null') {
			if ($op->targ && substr(B::ppname($op->targ), 3) eq 'list') {
				my @kids = op_kids $op;
				for my $k (@kids) {
	...
	my sub nicer_errors {
		...
		if ($op->name eq 'null' or $op->name eq 'rv2sv' or $op->name eq 'rv2av' or $op->name eq 'rv2hv' or $op->name eq 'rv2gv') {
